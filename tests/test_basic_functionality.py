...
import pytest
import yaml
from layered_config_tree import (  # type: ignore
    ConfigNode,
    ConfigurationError,
...
def full_node(layers_and_values: Dict[str, str]) -> ConfigNode:
    node = ConfigNode(list(layers_and_values.keys()), name="test_node")
    for layer, value in layers_and_values.items():
        node.update(value, layer, source=None)
    return node
...
def test_tree_coerce_dict() -> None:
    d: Dict[Optional[str], Optional[Any]]
    d, s = {}, "test"
